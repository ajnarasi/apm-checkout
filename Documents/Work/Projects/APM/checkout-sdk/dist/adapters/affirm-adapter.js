...
export function createAffirmAdapter() {
    let eventBus = null;
    let config = null;
    ...
    return {
        ...
        async handleRedirectReturn(queryParams) {
            ...
            if (checkoutToken)
                emit(UniversalEventType.PAYMENT_AUTHORIZED, { checkoutToken });
        },
        getServerHandoff() { return { endpoint: '/api/affirm/checkout', method: 'POST', body: { amount: numberToString(config?.amount.total ?? 0), currency: config?.amount.currency ?? 'USD' } }; },
...
//# sourceMappingURL=affirm-adapter.js.map