...
export function createGrabpayAdapter() {
    let eventBus = null;
    let config = null;
    ...
    return {
        ...
        async authorize(paymentData) {
            ...
            try {
                ...
                if (redirectUrl)
                    emit(UniversalEventType.REDIRECT_REQUIRED, { url: redirectUrl, transactionId: data.transactionProcessingDetails?.transactionId });
                else
        ...
        getServerHandoff() { return { endpoint: '/api/grabpay/charge', method: 'POST', body: { amount: multiply100(config?.amount.total ?? 0), currency: config?.amount.currency ?? 'USD' } }; },
...
//# sourceMappingURL=grabpay-adapter.js.map