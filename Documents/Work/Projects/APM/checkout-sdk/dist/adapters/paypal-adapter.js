    ...
    return {
        ...
        async authorize(paymentData) {
            ...
            try {
                ...
                if (redirectUrl)
                    emit(UniversalEventType.REDIRECT_REQUIRED, { url: redirectUrl, orderID: data._raw?.id });
                else
                    emit(UniversalEventType.PAYMENT_ERROR, { code: ErrorCode.AUTH_FAILED, message: 'No approve link in PayPal response', retryable: false });
            }
            catch (err) {
        ...
        getServerHandoff() { return { endpoint: '/api/paypal/order', method: 'POST', body: { amount: numberToString(config?.amount.total ?? 0), currency: config?.amount.currency ?? 'USD' } }; },
