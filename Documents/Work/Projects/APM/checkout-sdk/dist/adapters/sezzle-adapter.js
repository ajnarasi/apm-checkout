...
export function createSezzleAdapter() {
    let eventBus = null;
    let config = null;
    ...
    return {
        ...
        async authorize(paymentData) {
            ...
            try {
                ...
                if (redirectUrl)
                    emit(UniversalEventType.REDIRECT_REQUIRED, { url: redirectUrl, orderUuid: data._raw?.uuid });
                else
        ...
        async handleRedirectReturn(queryParams) {
            ...
            if (!orderUuid) {
                emit(UniversalEventType.PAYMENT_ERROR, { code: ErrorCode.REDIRECT_FAILED, message: 'Missing orderUuid', retryable: false });
                return;
            }
            emit(UniversalEventType.PAYMENT_AUTHORIZED, { orderUuid });
        },
        getServerHandoff() { return { endpoint: '/api/sezzle/checkout', method: 'POST', body: { amount: numberToString(config?.amount.total ?? 0), currency: config?.amount.currency ?? 'USD' } }; },
...
//# sourceMappingURL=sezzle-adapter.js.map