...
export function createTabapayAdapter() {
    let eventBus = null;
    let config = null;
    ...
    return {
        ...
        async authorize(paymentData) {
            ...
            try {
                ...
                if (redirectUrl)
                    emit(UniversalEventType.REDIRECT_REQUIRED, { url: redirectUrl, sessionId: data._raw?.sessionId });
                else
        ...
        async handleRedirectReturn(queryParams) {
            ...
            if (!sessionId) {
                emit(UniversalEventType.PAYMENT_ERROR, { code: ErrorCode.REDIRECT_FAILED, message: 'Missing sessionId', retryable: false });
                return;
            }
            emit(UniversalEventType.PAYMENT_AUTHORIZED, { sessionId });
        },
        getServerHandoff() { return { endpoint: '/api/tabapay/session', method: 'POST', body: { amount: numberToString(config?.amount.total ?? 0), currency: config?.amount.currency ?? 'USD' } }; },
...
//# sourceMappingURL=tabapay-adapter.js.map