...
export function createZeptoAdapter() {
    let eventBus = null;
    let config = null;
    ...
    return {
        ...
        async authorize(paymentData) {
            ...
            try {
                ...
                validateCurrency(paymentData.amount.currency ?? 'AUD');
            }
            catch (err) {
            ...
            try {
                ...
                if (redirectUrl)
                    emit(UniversalEventType.REDIRECT_REQUIRED, { url: redirectUrl, agreementId: data._raw?.uid });
                else
                    emit(UniversalEventType.PAYMENT_ERROR, { code: ErrorCode.AUTH_FAILED, message: 'No authorization URL in Zepto response', retryable: false });
            }
            catch (err) {
        ...
        async handleRedirectReturn(queryParams) {
            ...
            if (!agreementId) {
                emit(UniversalEventType.PAYMENT_ERROR, { code: ErrorCode.REDIRECT_FAILED, message: 'Missing agreement_uid', retryable: false });
                return;
            }
            if (status === 'cancelled') {
                emit(UniversalEventType.PAYMENT_CANCELLED, { agreementId });
                return;
            }
            emit(UniversalEventType.PAYMENT_AUTHORIZED, { agreementId, status: status || 'active' });
        },
        getServerHandoff() { return { endpoint: '/api/zepto/agreement', method: 'POST', body: { amount: multiply100(config?.amount.total ?? 0), currency: config?.amount.currency ?? 'AUD' } }; },
        mapConfig(chConfig) { return { amount: multiply100(chConfig.amount.total), currency: chConfig.amount.currency }; },
        teardown() { eventBus = null; config = null; lastResponse = null; initialized = false; },
    };
}
export const zeptoAdapter = createZeptoAdapter();
//# sourceMappingURL=zepto-adapter.js.map