...
export function createPaypalPaylaterAdapter(): APMAdapter {
  ...
  return {
    ...
    async authorize(paymentData: PaymentData): Promise<void> {
      ...
      try {
        ...
        if (redirectUrl) emit(UniversalEventType.PAYMENT_AUTHORIZED, { url: redirectUrl, orderID: data._raw?.id, paymentMethod: 'paylater' });
        else emit(UniversalEventType.PAYMENT_ERROR, { code: ErrorCode.AUTH_FAILED, message: 'No approve link in PayPal Pay Later response', retryable: false });
    ...
    getServerHandoff(): ServerHandoff { return { endpoint: '/api/paypal/paylater-order', method: 'POST', body: { amount: numberToString(config?.amount.total ?? 0), currency: config?.amount.currency ?? 'USD', paymentMethod: 'paylater' } }; },
    mapConfig(chConfig: CheckoutConfig) { return { amount: numberToString(chConfig.amount.total), currency: chConfig.amount.currency, enableFunding: 'paylater' }; },
