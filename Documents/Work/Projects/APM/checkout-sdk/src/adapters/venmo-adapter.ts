...
export function createVenmoAdapter(): APMAdapter {
  ...
  return {
    ...
    async authorize(paymentData: PaymentData): Promise<void> {
      ...
      try {
        ...
        if (redirectUrl) emit(UniversalEventType.REDIRECT_REQUIRED, { url: redirectUrl, nonce: data._raw?.nonce });
    ...
    async handleRedirectReturn(queryParams: Record<string, string>): Promise<void> {
      ...
      if (!nonce) { emit(UniversalEventType.PAYMENT_ERROR, { code: ErrorCode.REDIRECT_FAILED, message: 'Missing nonce', retryable: false }); return; }
      emit(UniversalEventType.PAYMENT_AUTHORIZED, { nonce });
    },
    getServerHandoff(): ServerHandoff { return { endpoint: '/api/venmo/tokenize', method: 'POST', body: { amount: numberToString(config?.amount.total ?? 0), currency: config?.amount.currency ?? 'USD' } }; },
