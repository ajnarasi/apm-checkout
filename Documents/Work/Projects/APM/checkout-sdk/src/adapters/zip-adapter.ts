...
export function createZipAdapter(): APMAdapter {
  ...
  return {
    ...
    async handleRedirectReturn(queryParams: Record<string, string>): Promise<void> {
      ...
      if (checkoutId) emit(UniversalEventType.PAYMENT_AUTHORIZED, { checkoutId });
    },
    getServerHandoff(): ServerHandoff { return { endpoint: '/api/zip/checkout', method: 'POST', body: { amount: numberToString(config?.amount.total ?? 0), currency: config?.amount.currency ?? 'USD' } }; },
