...
| **Brand Color** | #1677FF | #1677FF |
| **Button Styles** | Standard PPRO button | — |
| **Logo URL** | /logos/alipay.svg | — |
...
3. render(container) → Render branded "Alipay" button (#1677FF)
4. authorize(paymentData) → POST /api/ppro/charge with paymentMethod=ALIPAY
...
6. getServerHandoff() → { endpoint: '/api/ppro/charge', method: 'POST', body: { paymentMethod: 'ALIPAY', amount, currency: 'CNY' } }
...
| paymentMethod.provider | paymentMethod | MAP_ENUM (→ ALIPAY) | Yes |
