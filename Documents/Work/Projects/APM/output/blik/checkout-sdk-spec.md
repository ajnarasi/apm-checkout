# BLIK — Checkout SDK Integration Spec
...
|---|---|---|
| **Button Label** | "Pay with BLIK" | "Pay with BLIK" |
| **Brand Color** | #000000 | #000000 |
| **Button Styles** | Standard PPRO button | — |
| **Logo URL** | /logos/blik.svg | — |
| **Input Field** | 6-digit BLIK code (numeric) | — |
...
3. render(container) → Render branded "BLIK" button (#000000)
4. authorize(paymentData) → POST /api/ppro/charge with paymentMethod=BLIK
...
6. getServerHandoff() → { endpoint: '/api/ppro/charge', method: 'POST', body: { paymentMethod: 'BLIK', amount, currency: 'PLN' } }
...
| paymentMethod.provider | paymentMethod | MAP_ENUM (→ BLIK) | Yes |
