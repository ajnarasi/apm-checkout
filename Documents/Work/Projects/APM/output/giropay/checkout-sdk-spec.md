# Giropay — Checkout SDK Integration Spec
...
|---|---|---|
| **Button Label** | "Pay with Giropay" | "Pay with Giropay" |
| **Brand Color** | #003A7D | #003A7D |
| **Button Styles** | Standard PPRO button | — |
| **Logo URL** | /logos/giropay.svg | — |
...
3. render(container) → Render branded "Giropay" button (#003A7D)
4. authorize(paymentData) → POST /api/ppro/charge with paymentMethod=GIROPAY
...
6. getServerHandoff() → { endpoint: '/api/ppro/charge', method: 'POST', body: { paymentMethod: 'GIROPAY', amount, currency: 'EUR' } }
...
| paymentMethod.provider | paymentMethod | MAP_ENUM (→ GIROPAY) | Yes |
