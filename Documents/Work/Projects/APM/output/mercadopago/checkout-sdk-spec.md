# Mercado Pago — Checkout SDK Integration Spec

## Section A: SDK Metadata

| Property | Value |
|---|---|
| **APM Code** | MERCADOPAGO |
...
|---|---|---|
| AR | ARS | Primary market |

### Button/UI Configuration

| Property | Options | Default |
|---|---|---|
| **Button Label** | "Pay with Mercado Pago" | "Pay with Mercado Pago" |
| **Brand Color** | #009EE3 | #009EE3 |
| **Button Styles** | Standard PPRO button | — |
| **Logo URL** | /logos/mercadopago.svg | — |
...
|---|---|---|
| Redirect | Yes | Wallet app redirect |
...
3. render(container) → Render branded "Mercado Pago" button (#009EE3)
4. authorize(paymentData) → POST /api/ppro/charge with paymentMethod=MERCADOPAGO
   → Emits: PAYMENT_METHOD_READY
   → Then: PPRO charge → wallet redirect → customer approves → returns to merchant
5. handleRedirectReturn(params) → GET /api/ppro/charge/{chargeId} → check status
6. getServerHandoff() → { endpoint: '/api/ppro/charge', method: 'POST', body: { paymentMethod: 'MERCADOPAGO', amount, currency: 'ARS' } }
...
| paymentMethod.provider | paymentMethod | MAP_ENUM (→ MERCADOPAGO) | Yes |
