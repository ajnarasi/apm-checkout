# MobilePay — Checkout SDK Integration Spec
...
|---|---|---|
| DK | DKK | Primary market |

### Button/UI Configuration

| Property | Options | Default |
|---|---|---|
| **Button Label** | "Pay with MobilePay" | "Pay with MobilePay" |
| **Brand Color** | #5A78FF | #5A78FF |
| **Button Styles** | Standard PPRO button | — |
| **Logo URL** | /logos/mobilepay.svg | — |
...
3. render(container) → Render branded "MobilePay" button (#5A78FF)
4. authorize(paymentData) → POST /api/ppro/charge with paymentMethod=MOBILEPAY
...
6. getServerHandoff() → { endpoint: '/api/ppro/charge', method: 'POST', body: { paymentMethod: 'MOBILEPAY', amount, currency: 'DKK' } }
...
| paymentMethod.provider | paymentMethod | MAP_ENUM (→ MOBILEPAY) | Yes |
