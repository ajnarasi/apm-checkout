# Multibanco — Checkout SDK Integration Spec
...
|---|---|---|
| **Button Label** | "Pay with Multibanco" | "Pay with Multibanco" |
| **Brand Color** | #0066CC | #0066CC |
| **Button Styles** | Standard PPRO button | — |
| **Logo URL** | /logos/multibanco.svg | — |
...
3. render(container) → Render branded "Multibanco" button (#0066CC)
4. authorize(paymentData) → POST /api/ppro/charge with paymentMethod=MULTIBANCO
...
6. getServerHandoff() → { endpoint: '/api/ppro/charge', method: 'POST', body: { paymentMethod: 'MULTIBANCO', amount, currency: 'EUR' } }
...
| paymentMethod.provider | paymentMethod | MAP_ENUM (→ MULTIBANCO) | Yes |
