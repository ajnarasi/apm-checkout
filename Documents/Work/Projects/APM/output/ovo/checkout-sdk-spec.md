# OVO — Checkout SDK Integration Spec
...
|---|---|---|
| **Button Label** | "Pay with OVO" | "Pay with OVO" |
| **Brand Color** | #4C2A86 | #4C2A86 |
| **Button Styles** | Standard PPRO button | — |
| **Logo URL** | /logos/ovo.svg | — |
...
3. render(container) → Render branded "OVO" button (#4C2A86)
4. authorize(paymentData) → POST /api/ppro/charge with paymentMethod=OVO
...
6. getServerHandoff() → { endpoint: '/api/ppro/charge', method: 'POST', body: { paymentMethod: 'OVO', amount, currency: 'IDR' } }
...
| paymentMethod.provider | paymentMethod | MAP_ENUM (→ OVO) | Yes |
