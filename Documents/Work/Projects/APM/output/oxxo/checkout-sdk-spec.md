# OXXO — Checkout SDK Integration Spec
...
|---|---|---|
| MX | MXN | Primary market |

### Button/UI Configuration

| Property | Options | Default |
|---|---|---|
| **Button Label** | "Pay with OXXO" | "Pay with OXXO" |
| **Brand Color** | #CD1B2A | #CD1B2A |
| **Button Styles** | Standard PPRO button | — |
| **Logo URL** | /logos/oxxo.svg | — |
...
3. render(container) → Render branded "OXXO" button (#CD1B2A)
4. authorize(paymentData) → POST /api/ppro/charge with paymentMethod=OXXO
...
6. getServerHandoff() → { endpoint: '/api/ppro/charge', method: 'POST', body: { paymentMethod: 'OXXO', amount, currency: 'MXN' } }
...
| paymentMethod.provider | paymentMethod | MAP_ENUM (→ OXXO) | Yes |
