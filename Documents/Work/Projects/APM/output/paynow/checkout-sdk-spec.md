# PayNow — Checkout SDK Integration Spec
...
|---|---|---|
| **Button Label** | "Pay with PayNow" | "Pay with PayNow" |
| **Brand Color** | #7B2D8E | #7B2D8E |
| **Button Styles** | Standard PPRO button | — |
| **Logo URL** | /logos/paynow.svg | — |
...
3. render(container) → Render branded "PayNow" button (#7B2D8E)
4. authorize(paymentData) → POST /api/ppro/charge with paymentMethod=PAYNOW
...
6. getServerHandoff() → { endpoint: '/api/ppro/charge', method: 'POST', body: { paymentMethod: 'PAYNOW', amount, currency: 'SGD' } }
...
| paymentMethod.provider | paymentMethod | MAP_ENUM (→ PAYNOW) | Yes |
