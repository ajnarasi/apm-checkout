# PayPay — Checkout SDK Integration Spec
...
|---|---|---|
| JP | JPY | Primary market |

### Button/UI Configuration

| Property | Options | Default |
|---|---|---|
| **Button Label** | "Pay with PayPay" | "Pay with PayPay" |
| **Brand Color** | #FF0033 | #FF0033 |
| **Button Styles** | Standard PPRO button | — |
| **Logo URL** | /logos/paypay.svg | — |
| **Dual Mode** | QR on desktop, redirect on mobile | — |
...
| QR_CODE_GENERATED | PPRO returns SCAN_CODE auth with QR data | { qrCodeUrl, chargeId } |
...
3. render(container) → Render branded "PayPay" button (#FF0033)
4. authorize(paymentData) → POST /api/ppro/charge with paymentMethod=PAYPAY
   → Emits: PAYMENT_METHOD_READY
   → Then: PPRO charge → QR code generated → customer scans → polling confirms payment
5. handleRedirectReturn(params) → GET /api/ppro/charge/{chargeId} → check status
6. getServerHandoff() → { endpoint: '/api/ppro/charge', method: 'POST', body: { paymentMethod: 'PAYPAY', amount, currency: 'JPY' } }
...
| Polling | 3,000ms interval / 60 min max
5 min interval / varies by APM | Exponential backoff after 10 polls |
...
| paymentMethod.provider | paymentMethod | MAP_ENUM (→ PAYPAY) | Yes |
