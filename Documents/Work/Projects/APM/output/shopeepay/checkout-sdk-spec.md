# ShopeePay — Checkout SDK Integration Spec
...
|---|---|---|
| ID | IDR | Primary market |

### Button/UI Configuration

| Property | Options | Default |
|---|---|---|
| **Button Label** | "Pay with ShopeePay" | "Pay with ShopeePay" |
| **Brand Color** | #EE4D2D | #EE4D2D |
| **Button Styles** | Standard PPRO button | — |
| **Logo URL** | /logos/shopeepay.svg | — |
...
3. render(container) → Render branded "ShopeePay" button (#EE4D2D)
4. authorize(paymentData) → POST /api/ppro/charge with paymentMethod=SHOPEEPAY
...
6. getServerHandoff() → { endpoint: '/api/ppro/charge', method: 'POST', body: { paymentMethod: 'SHOPEEPAY', amount, currency: 'IDR' } }
...
| paymentMethod.provider | paymentMethod | MAP_ENUM (→ SHOPEEPAY) | Yes |
