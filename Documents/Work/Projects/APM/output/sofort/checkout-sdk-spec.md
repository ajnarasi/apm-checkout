# Sofort — Checkout SDK Integration Spec
...
|---|---|---|
| **Button Label** | "Pay with Sofort" | "Pay with Sofort" |
| **Brand Color** | #EF809F | #EF809F |
| **Button Styles** | Standard PPRO button | — |
| **Logo URL** | /logos/sofort.svg | — |
...
3. render(container) → Render branded "Sofort" button (#EF809F)
4. authorize(paymentData) → POST /api/ppro/charge with paymentMethod=SOFORT
...
6. getServerHandoff() → { endpoint: '/api/ppro/charge', method: 'POST', body: { paymentMethod: 'SOFORT', amount, currency: 'EUR' } }
...
| paymentMethod.provider | paymentMethod | MAP_ENUM (→ SOFORT) | Yes |
