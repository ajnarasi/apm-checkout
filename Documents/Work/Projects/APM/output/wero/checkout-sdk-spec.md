# Wero — Checkout SDK Integration Spec
...
|---|---|---|
| **Button Label** | "Pay with Wero" | "Pay with Wero" |
| **Brand Color** | #4E89FF | #4E89FF |
| **Button Styles** | Standard PPRO button | — |
| **Logo URL** | /logos/wero.svg | — |
...
3. render(container) → Render branded "Wero" button (#4E89FF)
4. authorize(paymentData) → POST /api/ppro/charge with paymentMethod=WERO
...
6. getServerHandoff() → { endpoint: '/api/ppro/charge', method: 'POST', body: { paymentMethod: 'WERO', amount, currency: 'EUR' } }
...
| paymentMethod.provider | paymentMethod | MAP_ENUM (→ WERO) | Yes |
